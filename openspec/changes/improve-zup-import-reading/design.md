...
6. **Use a hybrid deterministic + VLM pipeline.**
   ...
   - Payment taxonomy: import rows stay atomic, but the importer also writes `Импорт_1С_СтруктураВыплат`, a dynamic per-company JSON taxonomy of section/category/kind combinations and their calculation roles. `Доплата до оклада` is segregated as its own category with role `salary_top_up`.
   - Quality gates: extracted rows carry organization, employee, accrual date, and period; quality output verifies organization, employee, and contiguous monthly coverage without blocking the pipeline. Mismatches, blank organization, raw employee variants, file warnings, data-completeness metrics, and explanations for partial salary months are written to `Импорт_1С_QG` for manual verification.
   - Observability: if `LANGFUSE_BASE_URL`, `LANGFUSE_PUBLIC_KEY`, and `LANGFUSE_SECRET_KEY` are configured in Script properties, every Polza VLM extraction emits a Langfuse trace/generation, and every QG write emits a compact quality-gate trace. `Импорт_1С_VLM` stores the trace id beside the raw JSON for quick cross-checking.
   - Reconstruction semantics: salary uses payroll-slip factual worked days from salary rows, Consultant production-calendar month workdays, and salary-base amounts including `Доплата до оклада` as an окладное доначисление. Premium sheets use accrued premiums by accrual period; vacation rows are built from accrued vacation rows and matched to vacation payment rows within the same source file. Payment statement names/dates are written to auxiliary columns so Art. 236 compensation can use the actual statement date. After reconstruction fill, `Из_1С_*` sheets are recalculated through the same indexation engine.
...
- Local Langfuse often runs on `localhost`, which Google Apps Script cannot reach from Google's servers → use an externally reachable tunnel/proxy URL in `LANGFUSE_BASE_URL`; tracing is optional and failures are logged without blocking import.
