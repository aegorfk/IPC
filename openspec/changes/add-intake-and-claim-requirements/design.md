...
The all-sheets workflow is one coordinated calculation transaction. It acquires the document lock (falling back to the script lock), resolves every calculation sheet once into a descriptor containing the normalized layout, actual header row, semantic columns, and sheet, and snapshots every adapter-owned output plus the authoritative audit extent and relevant document properties before the first mutation. Base restoration, base calculations, recovery and derivative writes, final rescan, audit rendering, flush, and registry/selection persistence execute inside that boundary. A fatal Sheets, property, audit, or flush failure restores values, formulas, notes, backgrounds, number formats, audit extent, and properties before the error is rethrown. Per-sheet data-quality failures remain review warnings and do not authorize partial writes to their dependent vacation outputs.
