...
The generated-Docs history is an independent dynamic side block in columns G:I. `CLAIM_INTAKE_DOCS_HISTORY` is its sole extent authority: a missing, malformed, or wrong-sheet range initializes only the configured first row and never scans other rows or adopts unrelated G:I content. Occupied non-history cells at the configured/next append row cause a safe failure. Legacy A:C cells are neither migrated nor cleared. The audit owns only the cells identified by `CLAIM_INTAKE_CLAIM_SELECTIONS`; that named range is likewise the sole authority for its previously rendered extent. Rendering expands the sheet only when required, clears only the authoritative previous range and the new target range, batches checkbox validation by claim family, and then resizes the named range. Unrelated cells above, below, or beside either named range are never migration candidates.
