...
- [x] 6.4 Add partial recovery and derivative-payment warning tests, including split salary liability schedules, per-source temporal recovery eligibility, full-recovery reversibility, semantic reordered multi-sheet discovery, exact one-pass descriptor reuse during recovery/derivative rescan, current-run vacation dependencies and exact source-row trace, nonfatal protected formulas with unrelated successful writes, fatal-by-default core exceptions versus typed data-quality warnings, stable setup before financial rollback, no automatic legacy deletion, 1000-row batched snapshot/restore ceilings, disputed unallocated audit rows, unchanged unsupported recovery-timing indexation, preload/discovery call ceilings, lock fallback/contention, complete planned-audit-union restoration including validations, and rollback injection at derivative, registry, audit, and flush boundaries.
